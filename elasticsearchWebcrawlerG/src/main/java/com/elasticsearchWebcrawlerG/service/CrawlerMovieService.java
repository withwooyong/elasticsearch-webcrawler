...
@Service
public class CrawlerMovieService {
...
//	private static String IMAGE_PATH = "D:\\sts-3.7.3.RELEASE\\workspace\\crawler\\src\\main\\java\\org\\test\\data\\img";
	...
	public MovieDTO getBasic(String url, String code, MovieDTO dto) throws Exception {
		...
		String imageName = imageDownload(poster.attr("src").toString(), image_gubun.POSTER.name(), code);
		//String imageName = poster.attr("src").toString();
		dto.setPoster(imageName);
		log.info("setPoster={}", imageName);
	...
	public MovieDTO getDetail(String url, String code, MovieDTO dto) throws Exception {
		...
		for (Element element : lst_people) {
			if (StringUtils.isNotEmpty(element.select(".k_name").text())
				...
				//String imageName = imageDownload(img_url, image_gubun.ACTOR.name(), actor_code);
				//actors_image.add(imageName); // 이미지
			}
		}
		...
		//String imageName = imageDownload(img_url, image_gubun.DIRECTOR.name(), director_code);
		//dto.setDirectorImg(imageName);
	...
	private String imageDownload(String imgUrl, String gubun, String code) throws Exception {
		...
		String imageName = gubun + "_" + code + "."+ext;
		ImageIO.write(img, ext, new File(IMAGE_PATH + "/" + imageName));
		
		log.info("user.dir={} IMAGE_PATH={} imageName={}", System.getProperty("user.dir"), IMAGE_PATH, imageName);
